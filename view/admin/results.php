<?php
    ...
    $sql = "SELECT fi.Nombre AS Competencia, fi.Id_ficha_instructor, f.Numero_ficha, fi.Competencia as Nombre_competencia, fi.Id_usuario, u.Nombre as Nombre, u.Apellido as Apellido, r.Respuesta AS Tipo_Respuesta, COUNT(r.Respuesta) AS Total_Respuestas, u.Tipo_documento, u.Numero_documento, f.Nombre_ficha as Formacion, p.Pregunta FROM respuesta r JOIN ficha_instructor fi ON r.Id_ficha_instructor = fi.Id_ficha_instructor JOIN ficha f ON fi.Id_ficha = f.Id_ficha JOIN pregunta p ON r.Id_pregunta = p.Id_pregunta JOIN usuario u ON fi.Id_usuario = u.Id_usuario WHERE p.Tipo_pregunta IN ('Si/No', 'Excelente/buena/Regular/Mala/Pesima') AND f.Numero_ficha LIKE '%$search_number_ficha%' AND u.Numero_documento LIKE '%$search_number_documento%' GROUP BY fi.Nombre, r.Respuesta, r.Id_pregunta ORDER BY fi.Nombre, r.Id_ficha_instructor, r.Id_pregunta ASC LIMIT $start_from, $results_per_page";
